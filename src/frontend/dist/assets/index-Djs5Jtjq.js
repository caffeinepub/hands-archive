...
function GraphView({ onSelectItem }) {
  ...
  const transformRef = reactExports.useRef({ x: 0, y: 0, k: 1 });
  ...
  const startLoop = reactExports.useCallback(
    (ctx, _canvas) => {
      ...
      function loop() {
        const { W, H } = sizeRef.current;
        if (W > 0 && H > 0) {
          const shouldSimulate = frameRef.current < 350 || dragNodeRef.current !== null;
          if (shouldSimulate) {
            tickSimulation(nodesRef.current, linksRef.current, W, H);
            frameRef.current++;
            if (frameRef.current === 300 && !hasFitRef.current && geometryRef.current === "standard") {
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  ...
                  children: "Scroll to zoom · Drag to pan · Click node to view"
                }
              )
            ]
          }
